...
function PuzzleIcon({ className }: { className?: string }) {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"
      ...
      className={className ?? "w-5 h-5 text-brand-purple shrink-0"}>
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-1 sm:px-6">
        <div className="grid grid-cols-4 divide-x divide-brand-dark/10">
          {trustBar.items.map((p, i) => {
            const Icon = ICONS[i] ?? PuzzleIcon;
            return (
              <div
                key={i}
                className="flex flex-col items-center gap-1 py-3 px-1 text-center sm:flex-row sm:items-center sm:gap-3 sm:py-4 sm:px-6 sm:text-left"
              >
                <Icon className="w-4 h-4 sm:w-6 sm:h-6 text-brand-purple shrink-0" />
                <div>
                  <div className="text-brand-dark font-semibold text-[9px] leading-tight sm:text-sm font-inter">
                    {p.title}
                  </div>
                  <div className="hidden sm:block text-brand-dark/55 text-xs font-inter mt-0.5">
                    {p.sub}
                  </div>
                </div>
              </div>
